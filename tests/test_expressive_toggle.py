...
from openagents.agents import Agent, AgentSession, ExpressiveOptions
from openagents.agents.llm.chat_context import ChatContext
from openagents.agents.voice.generation import (
...
MARKED_UP = (
    ...
    "Glad you called again. Docs: [the guide](https://docs.openagents.io)."
)
...
def test_strip_assistant_markup() -> None:
    ...
    assert "[the guide](https://docs.openagents.io)" in (assistant_texts[0] or "")
...
async def test_expressive_off_turn_scrubs_history() -> None:
    ...
    assert any("[the guide](https://docs.openagents.io)" in (t or "") for t in assistant_texts)
