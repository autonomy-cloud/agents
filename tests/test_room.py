...
These tests require a running OpenAgents server (bootstrapped via the openagents_server fixture).
"""
...
from openagents.agents.utils.participant import (
    wait_for_agent,
    wait_for_participant,
    wait_for_track_publication,
)
from openagents.agents.voice.room_io._output import _ParticipantAudioOutput

from .openagents_server import OPENAGENTS_API_KEY, OPENAGENTS_API_SECRET, OPENAGENTS_URL, openagents_server  # noqa: F401
from .utils.audio_test import AudioEnergyMonitor, SineToneSource
from .utils.openagents_test import (
...
def _make_token(
    ...
    *,
    ...
    token = (
        api.AccessToken(OPENAGENTS_API_KEY, OPENAGENTS_API_SECRET)
...
async def connect_room(
    ...
    await room.connect(OPENAGENTS_URL, token)
    try:
        yield room
    finally:
...
@pytest.mark.usefixtures("openagents_server")
...
@pytest.mark.skipif(
    not os.environ.get("OPENAGENTS_URL"),
    reason="OPENAGENTS_URL not set; skipping reconnect E2E tests "
    "(set OPENAGENTS_URL/OPENAGENTS_API_KEY/OPENAGENTS_API_SECRET to enable)",
)
@pytest.mark.skipif(
    not hasattr(rtc, "SimulateScenarioKind"),
    reason="openagents-rtc lacks SimulateScenarioKind; run `make link-rtc-local`",
)
class TestReconnect:
