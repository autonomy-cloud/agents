...
from openagents.agents.utils import AudioBuffer, is_given
        ...
        *,
        ...
        Args:
            ...
            noise_reduction_type: Type of noise reduction to apply. "near_field" or "far_field"
                This isn't needed when using OpenAgents's noise cancellation.
            ...
            vad: Optional Voice Activity Detector used to commit the audio buffer when the model
                ...
                `gpt-live-transcribe`).
                This must be supplied explicitly for such models. Pass `vad=None` to drive
        ...
        if use_realtime and _is_realtime_only(model):
            ...
            if not is_given(vad):
                raise ValueError(
                    "realtime transcription requires an explicit local VAD or vad=None"
                )
