...
from openagents.agents import DEFAULT_API_CONNECT_OPTIONS, vad
from openagents.agents.stt import SpeechEvent, SpeechEventType
...
def _internal_openai_endpoint(
    monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest
) -> None:
    monkeypatch.setenv("OPENAGENTS_OPENAI_BASE_URL", "http://models.internal/v1")
    if request.node.name == "test_switching_to_a_client_commit_model_needs_a_vad":
        return

    original_init = stt.STT.__init__

    def _explicit_test_vad(self: stt.STT, *args: Any, **kwargs: Any) -> None:
        kwargs.setdefault("vad", None)
        original_init(self, *args, **kwargs)

    monkeypatch.setattr(stt.STT, "__init__", _explicit_test_vad)
...
def test_a_client_commit_model_requires_an_explicit_vad() -> None:
    # The absorbed runtime never constructs hosted inference components implicitly.
    instance = stt.STT(api_key="test-key", model="gpt-live-transcribe", vad=None)
