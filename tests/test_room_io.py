...
from openagents.agents import utils
from openagents.agents.voice.room_io._input import (
    _ParticipantAudioInputStream,
    _ParticipantInputStream,
)
from openagents.agents.voice.room_io._output import (
    _ParticipantAudioOutput,
    _ParticipantStreamTranscriptionOutput,
    _ParticipantTranscriptionOutput,
)
from openagents.agents.voice.room_io.room_io import RoomIO
from openagents.agents.voice.room_io.types import NoiseCancellationParams
...
class _FakeRoom:
    def __init__(self) -> None:
        ...
        self._server_url = "wss://test.openagents.cloud"
...
async def test_transcription_output_strips_markup_but_keeps_links() -> None:
    ...
    await output.capture_text(
        '<expr type="expression" label="happy"/>See [the docs](https://docs.openagents.io)'
    )
    output.flush()
    ...
    assert "[the docs](https://docs.openagents.io)" in published
...
async def test_direct_processor_lifecycle() -> None:
    ...
    with patch("openagents.rtc.AudioStream.from_track", side_effect=lambda **kw: _MockAudioStream()):
...
async def test_audio_output_playback_started_fires_once_across_pause_resume() -> None:
    ...
    with patch("openagents.rtc.AudioSource", _FakeAudioSource):
