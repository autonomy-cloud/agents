...
4. Concurrency control, via two markers and a switch:
   ...
   - `--concurrent` (or `OPENAGENTS_TEST_CONCURRENCY=all`) runs *every* async test concurrently except
     `no_concurrent` ones; the default runs only `concurrent` ones; `--no-concurrent` (or
     `OPENAGENTS_TEST_CONCURRENCY=0`) forces everything sequential.
...
_ENV_TOGGLE = "OPENAGENTS_TEST_CONCURRENCY"
        ...
        if _xdist_active(config):
            ...
            if _requested_mode(config) == "all":
                raise pytest.UsageError(
                    "--concurrent (or OPENAGENTS_TEST_CONCURRENCY=all) is incompatible with pytest-xdist "
