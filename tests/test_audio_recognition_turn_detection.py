...
from openagents.agents import LanguageCode, vad
from openagents.agents.utils import aio
from openagents.agents.voice.audio_recognition import AudioRecognition
from openagents.agents.voice.turn import (
    TurnDetectionEvent,
...
class TestPredictionFutureLifecycle:
    ...
    async def test_late_stt_final_after_flush_short_circuits(
        ...
        caplog.set_level(logging.WARNING, logger="openagents.agents")
        ar = _make_full_recognition_for_eou()
        ar._turn_detector_flushed = True
        chat_ctx = _make_chat_ctx_stub()
