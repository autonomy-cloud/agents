...
class KeytermDetectionOptions(TypedDict, total=False):
    ...
    llm: LLM | str | None
    """Explicit local or internal-endpoint LLM used for extraction."""
    turn_interval: int
...
def _resolve_detection_llm(configured: LLM | str | None) -> LLM | None:
    """Use only an explicitly constructed internal/local detection model."""
    if isinstance(configured, LLM):
        return configured
    if isinstance(configured, str):
        logger.warning("keyterm detection requires an explicit LLM object; skipping %r", configured)
    return None
