"""Reusable helpers for end-to-end OpenAgents room tests.

These are intentionally framework-light — they work on top of `openagents-rtc`
plus the `openagents_server` pytest fixture (see `tests/lk_server.py`) and are
...
from ..openagents_server import OPENAGENTS_API_KEY, OPENAGENTS_API_SECRET, OPENAGENTS_URL
...
class ServerConfig:
    """OpenAgents server credentials used by E2E test helpers."""
...
def lk_config() -> ServerConfig | None:
    """Resolve OpenAgents server credentials from the environment.

    Looks at `OPENAGENTS_URL` / `OPENAGENTS_API_KEY` / `OPENAGENTS_API_SECRET`.
    Returns `None` when `OPENAGENTS_URL` is unset — callers should treat this
    as "skip the test"; OpenAgents Cloud / dev-server creds are environment-
    ...
    those defaults if `OPENAGENTS_URL` happens to be unset *and* the fixture
    is in use.
    """
    url = os.environ.get("OPENAGENTS_URL")
    if not url:
        return None
    return ServerConfig(
        url=url,
        api_key=os.environ.get("OPENAGENTS_API_KEY", OPENAGENTS_API_KEY),
        api_secret=os.environ.get("OPENAGENTS_API_SECRET", OPENAGENTS_API_SECRET),
    )
...
def _resolve_config(config: ServerConfig | None) -> ServerConfig:
    ...
    return ServerConfig(url=OPENAGENTS_URL, api_key=OPENAGENTS_API_KEY, api_secret=OPENAGENTS_API_SECRET)
...
async def connect_room(
    ...
    *,
    ...
    (`OPENAGENTS_URL` / `OPENAGENTS_API_KEY` / `OPENAGENTS_API_SECRET`) are used,
