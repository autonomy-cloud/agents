...
from openagents.protocol.agent_pb import agent_session as agent_pb
...
class AgentSession(rtc.EventEmitter[EventTypes], Generic[Userdata_T]):
    ...
    def __init__(
        ...
        *,
        ...
        """`AgentSession` is the OpenAgents Agents runtime that glues together
        ...
        Args:
            ...
            expressive (bool | ExpressiveOptions, optional): Let the LLM steer how the
                ...
                the injected instructions. Requires ``openagents.agents.inference.TTS`` with
        ...
        raw_turn_detection: TurnDetectionMode | None = turn_handling.get("turn_detection", "vad")
        ...
        if isinstance(stt, str) or isinstance(llm, str) or isinstance(tts, str):
            raise ValueError(
                "string model identifiers require the removed OpenAgents Cloud inference gateway; "
                "pass explicit STT, LLM, and TTS objects"
            )

        self._stt = stt or None
        self._using_default_vad = False
        if not is_given(vad):
            vad = None
