"""Tests for the OpenAgents expression marker (expr) dialect.
...
from openagents.agents.tts._provider_format import (
...
def test_split_all_markup_keeps_square_brackets() -> None:
    ...
    text = 'Press [Enter], then read [the docs](https://docs.openagents.io). <sound value="sigh"/>'
    clean, tags = split_all_markup(text)
    assert clean == "Press [Enter], then read [the docs](https://docs.openagents.io). "
...
MIXED = (
    ...
    'read [the docs](https://docs.openagents.io), then 1 < 2. <break time="1s"/> '
