...
override order matches what the simulation system depends on:
  ...
  agent_name:  OPENAGENTS_AGENT_NAME_OVERRIDE, then the rtc_session decorator arg,
               then OPENAGENTS_AGENT_NAME
"""
...
from openagents.agents.cli import proto
from openagents.agents.cli.cli import _run_worker
from openagents.agents.worker import AgentServer
...
class TestCLIOverridesConstructorAndEnv:
    def test_cli_url_wins(self):
        with patch.dict(os.environ, {"OPENAGENTS_URL": "ws://env"}):
    ...
    def test_cli_keys_win(self):
        with patch.dict(
            os.environ, {"OPENAGENTS_API_KEY": "env-key", "OPENAGENTS_API_SECRET": "env-secret"}
        ):
    ...
    def test_env_fallback_when_no_constructor_no_cli(self):
        with patch.dict(os.environ, {"OPENAGENTS_URL": "ws://env"}, clear=False):
...
class TestAgentNamePrecedence:
    def test_override_wins_over_decorator_and_default(self):
        with patch.dict(
            os.environ,
            {"OPENAGENTS_AGENT_NAME_OVERRIDE": "override-agent", "OPENAGENTS_AGENT_NAME": "env-agent"},
            clear=True,
        ):
            server = _TestableServer()
    ...
    def test_decorator_wins_over_env_default(self):
        with patch.dict(os.environ, {"OPENAGENTS_AGENT_NAME": "env-agent"}, clear=True):
            server = _TestableServer()
...
class TestFullPrecedenceChain:
    def test_everything_set(self):
        with patch.dict(
            os.environ,
            {
                "OPENAGENTS_URL": "ws://env",
                "OPENAGENTS_API_KEY": "env-key",
                "OPENAGENTS_API_SECRET": "env-secret",
                "OPENAGENTS_AGENT_NAME": "env-agent",
            },
        ):
            server = _TestableServer(
            ...
            assert config["api_key"] == "cli-key"
            assert config["api_secret"] == "cli-secret"
            # no OPENAGENTS_AGENT_NAME_OVERRIDE set, so the explicit decorator arg wins
            # over the OPENAGENTS_AGENT_NAME default
            assert config["agent_name"] == "decorator-agent"
