...
def get_ffi_lib() -> ctypes.CDLL:
    ...
    res = importlib.resources.files("openagents.rtc.resources") / libname
...
class FfiClient:
    ...
    def request(self, req: proto_ffi.FfiRequest) -> proto_ffi.FfiResponse:
        if self._pid != os.getpid():
            raise RuntimeError(
                "openagents.rtc was used in a parent process before fork(); the native "
                "runtime cannot be used across fork(). Do not create or connect a Room "
                "(or use any openagents.rtc object) before forking child processes."
            )
