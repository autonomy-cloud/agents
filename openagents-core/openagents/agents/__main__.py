...
logger = logging.getLogger("openagents.agents")


def _discover_and_import_plugins() -> list[str]:
    try:
        import openagents.plugins as ns
    except ImportError:
        logger.warning("no openagents.plugins namespace found — nothing to download")
        return []

    attempted: list[str] = []
    for _finder, name, is_pkg in pkgutil.iter_modules(ns.__path__, prefix="openagents.plugins."):
        if not is_pkg:
            continue
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m openagents.agents",
        description="OpenAgents Agents utilities.",
    )
    sub = parser.add_subparsers(dest="command", required=True)
    sub.add_parser(
        "download-files",
        help="Discover installed openagents-plugin-* packages and run their download_files step.",
    )
