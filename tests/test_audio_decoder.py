...
from openagents.agents.stt import SpeechEventType
from openagents.agents.utils.audio import audio_frames_from_file
from openagents.agents.utils.codecs import AudioStreamDecoder, StreamBuffer
from openagents.agents.utils.misc import is_cloud
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    os.getenv("OPENAGENTS_API_KEY") is None
    or os.getenv("OPENAGENTS_API_SECRET") is None
    or not is_cloud(os.getenv("OPENAGENTS_URL", "")),
    reason="OpenAgents cloud credentials are invalid",
)
...
async def test_aclose_while_probing_is_not_an_error(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.DEBUG, logger="openagents.agents"):
        await decoder.aclose()
