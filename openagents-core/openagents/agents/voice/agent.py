...
class Agent:
    ...
    def update_options(
        ...
        *,
        ...
        Pass explicit model objects, or ``None`` to disable a model (overriding
        the session). If the agent is running, the swap applies to the live pipeline.
...
def _set_activity_task_info(
    ...
    *,
    ...
    setattr(task, "__openagents_agents_activity_task", info)


def _get_activity_task_info(task: asyncio.Task[Any]) -> _ActivityTaskInfo | None:
    return getattr(task, "__openagents_agents_activity_task", None)
