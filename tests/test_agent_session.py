...
from openagents.agents.llm.chat_context import ChatContext, ChatMessage
from openagents.agents.stt import SpeechData, SpeechEvent, SpeechEventType, STTError
from openagents.agents.utils import aio
from openagents.agents.voice.agent_activity import AgentActivity
from openagents.agents.voice.audio_recognition import AudioRecognition, _EndOfTurnInfo
from openagents.agents.voice.endpointing import BaseEndpointing
from openagents.agents.voice.events import FunctionToolsExecutedEvent
from openagents.agents.voice.io import PlaybackFinishedEvent
...
def test_on_enter_ignored_tools() -> None:
    ...
    from openagents.agents.voice.agent_activity import _OnEnterContextVar, _OnEnterData
...
async def test_stt_errors_use_unrecoverable_error_tolerance() -> None:
    from openagents.agents.voice.agent_session import SessionConnectOptions
...
async def test_stt_pipeline_recreates_stream_after_unrecoverable_error(
    ...
    from openagents.agents.voice import audio_recognition
    from openagents.agents.voice.audio_recognition import _STTPipeline
...
async def test_vad_fallback_uses_next_vad_inference_event(
    ...
    caplog.set_level(logging.INFO, logger="openagents.agents")
...
async def test_default_vad_is_not_auto_provisioned() -> None:
...
async def test_default_turn_detection_is_local_vad() -> None:
    """No turn_detection given selects local VAD endpointing."""
    from openagents.agents.voice.agent_session import AgentSession
...
async def test_stt_context_options_passthrough() -> None:
    ...
    session = AgentSession(
        vad=None,
        stt_context_options={"keyterms": ["OpenAgents"], "forward_chat_context": False},
    )
    try:
        opts = session._opts.stt_context_options
        assert opts["keyterms"] == ["OpenAgents"]
...
async def test_default_vad_uses_local_endpointing_defaults() -> None:
    from openagents.agents.voice.agent_session import AgentSession
...
async def test_explicit_endpointing_overrides_local_default_per_key() -> None:
    from openagents.agents.voice.agent_session import AgentSession
...
async def test_agent_turn_detection_override_resolves_endpointing_per_activity() -> None:
    ...
    # Session and agent both use the absorbed runtime's local VAD defaults.
    session = AgentSession(vad=FakeVAD(fake_user_speeches=[]))
    try:
        default_activity = AgentActivity(Agent(instructions="test"), session)
        assert default_activity.endpointing_opts["min_delay"] == 0.5
        assert default_activity.endpointing_opts["max_delay"] == 3.0
...
async def test_runtime_endpointing_opts_survive_handoff() -> None:
    ...
    try:
        ...
        # untouched key still gets the local VAD default
        assert activity.endpointing_opts["max_delay"] == 3.0
    finally:
        await session.aclose()
