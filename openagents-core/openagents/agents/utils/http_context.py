...
def _set_default_cert_env() -> None:
    ...
    (which inherit it) and the Rust openagents-rtc SDK, since both OpenSSL and
...
def http_session() -> aiohttp.ClientSession:
    ...
    if val is None:
        raise RuntimeError(
            ...
            "If you're running plugins outside the agent worker (e.g. tests or scripts), wrap your code with `async with openagents.agents.utils.http_context.open(): ...`. "  # noqa: E501
