...
from openagents.agents.voice.transcription.filters import filter_emoji, filter_markdown
from openagents.agents.voice.transcription.text_transforms import _apply_text_transforms, replace
...
This is a sentence. 这是一个中文句子。これは日本語の文章です。你好！OpenAgents是一个直播音频和视频应用程序和服务的平台，我们正在测试Markdown~~过滤~~。"""  # noqa: E501
...
This is a sentence. 这是一个中文句子。これは日本語の文章です。你好！OpenAgents是一个直播音频和视频应用程序和服务的平台，我们正在测试Markdown。"""  # noqa: E501
...
async def test_replace_across_chunk_sizes(chunk_size: int):
    ...
    transform = replace({"OpenAgents": "Lyve Kit", "SQL": "sequel", "boundary": "EDGE"})
    text = "OpenAgents uses SQL. openagents boundary test."
...
async def test_replace_case_sensitive(chunk_size: int):
    ...
    transform = replace({"OpenAgents": "Lyve Kit"}, case_sensitive=True)
    result = await _collect(
        transform(_stream_text("OpenAgents is great. openagents should stay.", chunk_size))
    )
    assert result == "Lyve Kit is great. openagents should stay."
...
async def test_replace_flushes_non_prefix_text_immediately():
    ...
    transform = replace({"OpenAgents": "Lyve Kit"})
...
async def test_replace_holds_only_potential_prefix():
    ...
    transform = replace({"OpenAgents": "Lyve Kit"})
    chunks = await _collect_chunks(transform(_stream_text("visit Open", 100)))
    # "visit " flushes immediately; only "Open" (a OpenAgents prefix) is held to the end
    assert chunks[0] == "visit "
    assert "".join(chunks) == "visit Open"
