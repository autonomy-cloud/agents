...
from openagents.agents.llm.utils import ThinkingTokenFilter, strip_thinking_tokens
from openagents.plugins.openai import LLMStream
...
def test_preserves_content_without_thinking_tokens() -> None:
    assert _collect_visible_text([None, "", "Hello from OpenAgents"]) == "Hello from OpenAgents"
...
def test_flattens_multiple_and_string_parts() -> None:
    ...
    content = strip_thinking_tokens(
        [{"type": "text", "text": "Hello "}, "from ", {"type": "text", "text": "OpenAgents"}],
        state,
    )
    assert content == "Hello from OpenAgents"
