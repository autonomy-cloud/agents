...
new RTCInterceptor({
    onPeerConnectionCreate: (peerConnection) => {
        ...
        // Measure the actual Teams encoder output, not just the source canvas.
        const videoStatsTimer = setInterval(async () => {
            ...
            try {
                ...
                stats.forEach((report) => {
                    ...
                    window.ws?.sendJson({
                        type: 'TeamsVideoQuality', peerConnectionId,
                        width: report.frameWidth, height: report.frameHeight,
                        fps: report.framesPerSecond, bytesSent: report.bytesSent,
                        framesEncoded: report.framesEncoded,
                        qualityLimitationReason: report.qualityLimitationReason,
                        targetBitrate: report.targetBitrate,
                    });
                });
            } catch (_) { /* The transport may close during the stats read. */ }
        }, 10000);
