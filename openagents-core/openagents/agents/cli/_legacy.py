...
These commands have moved to the OpenAgents CLI (``lk agent ...``), which drives the
...
class TextModeLogFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        if record.name != "openagents.agents":
            return True
...
def _run_console(
    *,
    ...
    c.print(
        ...
        "https://github.com/autonomy-cloud/agents/#setup",
        tag="Deprecated",
...
def _build_cli(server: AgentServer) -> typer.Typer:
    ...
    def _set_dev_mode(ctx: typer.Context) -> None:
        ...
        if ctx.invoked_subcommand in ("console", "dev"):
            os.environ["OPENAGENTS_DEV_MODE"] = "1"
    ...
    def console(
        *,
        ...
        log_level: Annotated[
            LogLevel,
            typer.Option(
                help="Set the log level", case_sensitive=False, envvar="OPENAGENTS_LOG_LEVEL"
            ),
        ] = _dev_log_default,
    ) -> None:
        """
        Run a [bold]OpenAgents Agents[/bold] in [yellow]console[/yellow] mode.
    ...
    def start(
        *,
        ...
        url: Annotated[
            str | None,  # noqa: UP007
            typer.Option(
                help="The WebSocket URL of your OpenAgents server or Cloud project.",
                envvar="OPENAGENTS_URL",
            ),
        ] = None,
        api_key: Annotated[
            str | None,  # noqa: UP007
            typer.Option(
                help="API key for authenticating with your OpenAgents server or Cloud project.",
                envvar="OPENAGENTS_API_KEY",
            ),
        ] = None,
        api_secret: Annotated[
            str | None,  # noqa: UP007
            typer.Option(
                help="API secret for authenticating with your OpenAgents server or Cloud project.",
                envvar="OPENAGENTS_API_SECRET",
            ),
        ] = None,
    ...
    def dev(
        *,
        ...
        """
        Run a [bold]OpenAgents Agents[/bold] in [yellow]development[/yellow] mode.
        ...
        *,
        ...
        def _simulate_job() -> None:
            ...
            async def simulate_job() -> None:
                async with api.OpenAgentsAPI(url, api_key, api_secret) as lk_api:
    ...
    def download_files() -> None:
        ...
        c.print(
            ...
            "deprecated as of 1.5.10. Run it directly against the openagents.agents module "
            "instead, e.g. `uv run -m openagents.agents download-files`.[/yellow]"
        )
        warnings.warn(
            "Invoking the download-files command via your agent script is deprecated "
            "as of 1.5.10. Run it directly against the openagents.agents module instead, "
            "e.g. `uv run -m openagents.agents download-files`.",
            DeprecationWarning,
            stacklevel=2,
        )
...
def run_app(server: AgentServer | WorkerOptions) -> None:
    ...
    warnings.warn(
        "the built-in Python CLI is deprecated; use the OpenAgents CLI (`lk agent ...`) "
        "or `python -m openagents.agents`. It will be removed in a future release.",
        DeprecationWarning,
        stacklevel=2,
    )
