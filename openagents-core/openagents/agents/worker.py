...
from openagents import api, rtc
from openagents.protocol import agent, agent_worker, models
...
class ServerOptions:
    ...
    By default it uses ``OPENAGENTS_AGENT_NAME`` from environment"""  # noqa: E501
    worker_type: WorkerType = WorkerType.ROOM
    """Whether to spin up an agent for each room or publisher."""
    max_retry: int = 16
    """Maximum number of times to retry connecting to OpenAgents."""
    ws_url: str | None = None
    """URL to connect to the OpenAgents server.

    By default it uses ``OPENAGENTS_URL`` from environment"""
    api_key: str | None = field(repr=False, default=None)
    """API key to authenticate with OpenAgents.

    By default it uses ``OPENAGENTS_API_KEY`` from environment"""
    api_secret: str | None = field(repr=False, default=None)
    """API secret to authenticate with OpenAgents.

    By default it uses ``OPENAGENTS_API_SECRET`` from environment"""
    ...
    Can also be set via the ``OPENAGENTS_LOG_LEVEL`` environment variable or
    ...
    """HTTP proxy used to connect to the OpenAgents server.
...
class AgentServer(utils.EventEmitter[EventTypes]):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._ws_url = ws_url or os.environ.get("OPENAGENTS_URL") or ""
        self._api_key = api_key or os.environ.get("OPENAGENTS_API_KEY") or ""
        self._api_secret = api_secret or os.environ.get("OPENAGENTS_API_SECRET") or ""

        self._worker_token = os.environ.get("OPENAGENTS_WORKER_TOKEN") or ""  # hosted agents
        self._deployment = os.environ.get("OPENAGENTS_AGENT_DEPLOYMENT") or ""  # hosted agents
    ...
    def rtc_session(
        ...
        *,
        ...
        def decorator(
            ...
            # precedence: the OPENAGENTS_AGENT_NAME_OVERRIDE env var (a platform-injected
            # force, e.g. from the lk simulation launcher) takes priority, then the
            # explicit agent_name arg, then the OPENAGENTS_AGENT_NAME env default.
            if os.environ.get("OPENAGENTS_AGENT_NAME_OVERRIDE"):
                self._agent_name = os.environ["OPENAGENTS_AGENT_NAME_OVERRIDE"]
            elif agent_name:
                self._agent_name = agent_name
            elif os.environ.get("OPENAGENTS_AGENT_NAME"):
                self._agent_name = os.environ["OPENAGENTS_AGENT_NAME"]
            else:
                self._agent_name = ""
    ...
    async def run(self, *, devmode: bool = False, unregistered: bool = False) -> None:
        ...
        Args:
            ...
            unregistered (bool, optional):
                ...
                to the central OpenAgents job dispatcher.
        ...
        async with self._lock:
            ...
            self._api: api.OpenAgentsAPI | None = None
            ...
            if not self._simulation:
                ...
                async def health_check(_: Any) -> web.Response:
                    ...
                    if self._connection_failed:
                        return web.Response(status=503, text="failed to connect to OpenAgents")
            ...
            if not unregistered:
                if not self._ws_url:
                    raise ValueError("ws_url is required, or set OPENAGENTS_URL environment variable")

                if not self._api_key:
                    raise ValueError(
                        "api_key is required, or set OPENAGENTS_API_KEY environment variable"
                    )

                if not self._api_secret:
                    raise ValueError(
                        "api_secret is required, or set OPENAGENTS_API_SECRET environment variable"
                    )
            ...
            if self._ws_url:
                os.environ["OPENAGENTS_URL"] = self._ws_url
            if self._api_key:
                os.environ["OPENAGENTS_API_KEY"] = self._api_key
            if self._api_secret:
                os.environ["OPENAGENTS_API_SECRET"] = self._api_secret
            if self._worker_token:
                os.environ["OPENAGENTS_WORKER_TOKEN"] = self._worker_token
            if self._deployment:
                os.environ["OPENAGENTS_AGENT_DEPLOYMENT"] = self._deployment
    ...
    async def aclose(self) -> None:
        async with self._lock:
            ...
            # running unregistered without a OpenAgents connection)
        ...
        while not self._closed:
            try:
                ...
                if retry_count >= self._max_retry:
                    ...
                    raise RuntimeError(
                        f"failed to connect to OpenAgents after {retry_count} attempts",
                    ) from None
                ...
                logger.warning(
                    f"failed to connect to OpenAgents, retrying in {retry_delay}s",
