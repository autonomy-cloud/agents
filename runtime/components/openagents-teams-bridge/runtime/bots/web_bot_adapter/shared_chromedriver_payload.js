...
class BotVideoOutputStream {
    constructor({
        ...
        // Preserve facial detail when the platform adapts the virtual camera.
        if (this.sourceVideoTrack) this.sourceVideoTrack.contentHint = "detail";
    ...
    _startVideoDrawingLoop() {
        ...
        // Follow decoded frames rather than dropping a 25/30fps source to 15fps.
        // RAF is only a fallback for browsers without video frame callbacks.
        const video = this.videoElement;
        const useVideoCallback = typeof video.requestVideoFrameCallback === "function";
        let lastMediaTime = null;
        const schedule = () => {
            if (useVideoCallback) {
                this.videoFrameCallbackId = video.requestVideoFrameCallback(drawFrame);
            } else {
                this.videoRafId = requestAnimationFrame(drawFrame);
            }
        };
        const drawFrame = (_timestamp, metadata) => {
            if (this.videoElement !== video || video.paused || video.ended) return;
            const mediaTime = metadata?.mediaTime ?? video.currentTime;
            const vw = video.videoWidth;
            const vh = video.videoHeight;
            if (vw && vh && mediaTime !== lastMediaTime) {
    ...
    async playMediaStream(mediaStream) {
        ...
        try{
            ...
            if (hasAudio) {
                this.mediaStreamAudioSource = this.getAudioContext().createMediaStreamSource(mediaStream);
...
class BotOutputManager {
    ...
    botOutputMediaStreamIsReady() {
        return this.botOutputMediaStream.getTracks().length > 0;
    }
