...
from openagents.api.access_token import Claims
from openagents.protocol import agent, models

from .log import logger
from .observability import Tagger
from .telemetry import otel_metrics
from .telemetry.traces import _BufferingHandler, _shutdown_telemetry
from .types import (
...
class JobContext:
    ...
    def _start_log_buffering(self) -> None:
        """Cloud log buffering is disabled in the air-gapped runtime."""
        return
    ...
    def tagger(self) -> Tagger:
        """Returns the local Tagger for adding tags and outcomes to the session.
    ...
    def api(self) -> api.OpenAgentsAPI:
        """Returns an OpenAgentsAPI for making API calls to OpenAgents.
        ...
        OPENAGENTS_URL, OPENAGENTS_API_KEY, and OPENAGENTS_API_SECRET so this API is always
        usable inside job entrypoints.
        """
        return api.OpenAgentsAPI(session=http_context.http_session())
    ...
    def simulation_context(self) -> SimulationContext | None:
        ...
        from openagents.protocol import agent_simulation as sim_pb
    ...
    def add_sip_participant(
        ...
        *,
        ...
        Make sure you have an outbound SIP trunk created in OpenAgents.
        See https://docs.openagents.io/sip/trunk-outbound/ for more information.
        """
        if self.is_fake_job():
    ...
    def transfer_sip_participant(
        ...
        See https://docs.openagents.io/sip/transfer-cold/ for more information.
        """
        if self.is_fake_job():
            logger.warning(
...
class JobRequest:
    ...
    async def accept(
        ...
        *,
        ...
        """Accept the job request, and start the job if the OpenAgents SFU assigns the job to our worker."""  # noqa: E501
        if not identity:
