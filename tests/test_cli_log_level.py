...
from openagents.agents.cli._legacy import _build_cli
from openagents.agents.worker import AgentServer, ServerEnvOption, ServerOptions
...
class TestStartCommandLogLevel:
    @patch("openagents.agents.cli.cli._run_worker")
    ...
    def test_env_var_overrides_default(self, mock_run_worker, runner):
        server = _make_server()
        app = _build_cli(server)
        result = runner.invoke(app, ["start"], env={"OPENAGENTS_LOG_LEVEL": "CRITICAL"})
        assert result.exit_code == 0
    ...
    def test_cli_arg_overrides_env_var(self, mock_run_worker, runner):
        ...
        result = runner.invoke(
            ...
            env={"OPENAGENTS_LOG_LEVEL": "CRITICAL"},
        )
        assert result.exit_code == 0
...
class TestDevCommandLogLevel:
    ...
    def test_env_var_overrides_default(self, mock_run_worker, runner):
        server = _make_server()
        app = _build_cli(server)
        result = runner.invoke(app, ["dev", "--no-reload"], env={"OPENAGENTS_LOG_LEVEL": "ERROR"})
        assert result.exit_code == 0
        assert mock_run_worker.call_args.kwargs["args"].log_level == "ERROR"
