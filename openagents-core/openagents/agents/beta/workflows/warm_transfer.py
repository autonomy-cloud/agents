    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            sip_trunk_id: ID of a pre-configured OpenAgents SIP outbound trunk used to
                originate the call. Falls back to the ``OPENAGENTS_SIP_OUTBOUND_TRUNK``
        ...
        if is_given(sip_trunk_id):
            ...
            self._sip_trunk_id = os.getenv("OPENAGENTS_SIP_OUTBOUND_TRUNK", None)
        if self._sip_trunk_id is None and self._sip_connection is None:
            raise ValueError(
                "`OPENAGENTS_SIP_OUTBOUND_TRUNK` environment variable, `sip_trunk_id`,"
                " or `sip_connection` must be set"
            )

        self._sip_number = (
            sip_number if is_given(sip_number) else os.getenv("OPENAGENTS_SIP_NUMBER", "")
        )
