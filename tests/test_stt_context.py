...
from openagents.agents.llm import LLM, ChatContext, ChatMessage, CollectedResponse, FunctionToolCall
from openagents.agents.stt import STT, RecognizeStream, SpeechEvent, STTCapabilities
from openagents.agents.types import NOT_GIVEN, APIConnectOptions, NotGivenOr
from openagents.agents.utils import AudioBuffer
from openagents.agents.voice.events import ConversationItemAddedEvent
from openagents.agents.voice.keyterm_detection import (
    _PENDING_TTL,
    KeytermDetector,
    _detect_keyterms,
    _format_input,
...
async def test_user_precedence_and_dedup() -> None:
    d = _detector(
        static_keyterms=["Acme", "Acme", "OpenAgents"],
        llm=_RecordingLLM(([], ["OpenAgents", "Foo"], [])),
    )
    assert d.static_keyterms == ["Acme", "OpenAgents"]
    await d._run_once(_ctx())  # an auto term equal to a user term is dropped
    assert [t for t, _ in _entries(d)] == ["Foo"]
    assert d.keyterms == ["Acme", "OpenAgents", "Foo"]
...
async def test_detection_llm_metrics_forwarded() -> None:
    ...
    from openagents.agents.metrics import LLMMetrics
...
def test_resolve_stt_context_options_passthrough() -> None:
    ...
    assert resolved["keyterms"] == ["OpenAgents"]
