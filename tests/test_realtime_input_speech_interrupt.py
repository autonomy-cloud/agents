...
from openagents.agents import Agent, AgentSession, TurnHandlingOptions, llm
from openagents.agents.voice.agent_activity import AgentActivity
from openagents.agents.voice.speech_handle import SpeechHandle
...
def _openagents_keys(monkeypatch: pytest.MonkeyPatch) -> None:
    # the eager TurnDetector() default reads these; keep construction hermetic
    monkeypatch.setenv("OPENAGENTS_API_KEY", "k")
    monkeypatch.setenv("OPENAGENTS_API_SECRET", "s")
...
def test_input_speech_started_keeps_uninterruptible_speech(
    ...
    with caplog.at_level(logging.ERROR, logger="openagents.agents"):
