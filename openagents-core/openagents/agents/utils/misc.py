...
def is_cloud(url: str) -> bool:
    ...
    if hostname is None:
        return False
    return hostname.endswith(".openagents.cloud") or hostname.endswith(".openagents.run")
...
def is_dev_mode() -> bool:
    ...
    Reads the ``OPENAGENTS_DEV_MODE`` environment variable.
    """
    return os.getenv("OPENAGENTS_DEV_MODE") == "1"


def is_hosted() -> bool:
    """Return whether the agent is hosted on OpenAgents Cloud."""
    return os.getenv("OPENAGENTS_REMOTE_EOT_URL") is not None
